/*******************************************************************************
 *                                                                              *
 * Author    :  Angus Johnson * Version   :  6.1.3 (float) - Experimental * Date
 *:  16 January 2014                                                 * Website
...
bool SlopesEqual(const FPoint pt1,
                 const FPoint pt2,
                 const FPoint pt3,
                 const FPoint pt4)
{
...
inline double TopX(TEdge& edge, const double currentY)
{
  return (currentY == edge.Top.Y)
           ? edge.Top.X
           : edge.Bot.X + edge.Dx * (currentY - edge.Bot.Y);
}
//------------------------------------------------------------------------------
...
bool GetOverlapSegment(
  FPoint pt1a, FPoint pt1b, FPoint pt2a, FPoint pt2b, FPoint& pt1, FPoint& pt2)
{
...
bool Pt2IsBetweenPt1AndPt3(const FPoint pt1, const FPoint pt2, const FPoint pt3)
{
...
bool HorzSegmentsOverlap(const FPoint& pt1a,
                         const FPoint& pt1b,
                         const FPoint& pt2a,
                         const FPoint& pt2b)
{
...
void Clipper::InsertScanbeam(const double Y) { m_Scanbeam.insert(Y); }
//------------------------------------------------------------------------------
...
OutPt* Clipper::AddLocalMinPoly(TEdge* e1, TEdge* e2, const FPoint& Pt)
{
  ...
  if (prevE && prevE->OutIdx >= 0 && (TopX(*prevE, Pt.Y) == TopX(*e, Pt.Y)) &&
      SlopesEqual(*e, *prevE) && (e->WindDelta != 0) &&
      (prevE->WindDelta != 0)) {
...
void Clipper::InsertLocalMinimaIntoAEL(const double botY)
{
  while (m_CurrentLM && (m_CurrentLM->Y == botY)) {
    ...
    if (lb->OutIdx >= 0 && lb->PrevInAEL &&
        ...
        SlopesEqual(*lb->PrevInAEL, *lb) && (lb->WindDelta != 0) &&
    ...
    if (lb->NextInAEL != rb) {
      ...
      if (rb->OutIdx >= 0 && rb->PrevInAEL->OutIdx >= 0 &&
          SlopesEqual(*rb->PrevInAEL, *rb) && (rb->WindDelta != 0) &&
...
void Clipper::IntersectEdges(TEdge*        e1,
                             TEdge*        e2,
                             const FPoint& Pt,
                             bool          protect)
{
  ...
  else if ((e1Wc == 0 || e1Wc == 1) && (e2Wc == 0 || e2Wc == 1) && !e1stops &&
           !e2stops) {
...
void Clipper::PrepareHorzJoins(TEdge* horzEdge, bool isTopOfScanbeam)
{
  ...
  for (JoinList::size_type i = 0; i < m_GhostJoins.size(); ++i) {
    ...
    if (HorzSegmentsOverlap(
          j->OutPt1->Pt, j->OffPt, horzEdge->Bot, horzEdge->Top))
  ...
  while (e) {
    if (IsIntermediate(e, topY)) {
      ...
      if (ePrev && ePrev->Curr.X == e->Bot.X && ePrev->Curr.Y == e->Bot.Y &&
          ...
          SlopesEqual(*e, *ePrev) && (e->WindDelta != 0) &&
          (ePrev->WindDelta != 0)) {
        ...
        AddJoin(op, op2, e->Top);
               ...
               eNext->Curr.Y > eNext->Top.Y && SlopesEqual(*e, *eNext) &&
...
bool GetOverlap(const double a1,
                const double a2,
                const double b1,
                const double b2,
                double&      Left,
...
bool Clipper::JoinPoints(Join* j, OutRec* outRec1, OutRec* outRec2)
{
  ...
  if (isHorizontal && (j->OffPt == j->OutPt1->Pt) &&
    ...
    while ((op1b->Pt == op1->Pt) && (op1b != op1))
      op1b = op1b->Next;
    bool Reverse1 =
      ((op1b->Pt.Y > op1->Pt.Y) || !SlopesEqual(op1->Pt, op1b->Pt, j->OffPt));
    if (Reverse1) {
      op1b = op1->Prev;
      while ((op1b->Pt == op1->Pt) && (op1b != op1))
        op1b = op1b->Prev;
    ...
    while ((op2b->Pt == op2->Pt) && (op2b != op2))
      op2b = op2b->Next;
    bool Reverse2 =
      ((op2b->Pt.Y > op2->Pt.Y) || !SlopesEqual(op2->Pt, op2b->Pt, j->OffPt));
    if (Reverse2) {
      op2b = op2->Prev;
      while ((op2b->Pt == op2->Pt) && (op2b != op2))
        op2b = op2b->Prev;
      ...
      if (node.m_jointype == jtRound) {
        ...
        for (double j = 1; j <= steps; j++) {
          m_destPoly.push_back(
            FPoint(m_srcPoly[0].X + X * delta, m_srcPoly[0].Y + Y * delta));
          double X2 = X;
          X = X * m_cos - m_sin * Y;
          Y = X2 * m_sin + Y * m_cos;
        }
...
void ClipperOffset::DoSquare(int j, int k)
{
  double dx = std::tan(std::atan2(m_sinA,
                       ...
                       4);
  m_destPoly.push_back(
    FPoint(m_srcPoly[j].X + m_delta * (m_normals[k].X - m_normals[k].Y * dx),
           m_srcPoly[j].Y + m_delta * (m_normals[k].Y + m_normals[k].X * dx)));
  m_destPoly.push_back(
    FPoint(m_srcPoly[j].X + m_delta * (m_normals[j].X + m_normals[j].Y * dx),
...
void ClipperOffset::DoMiter(int j, int k, double r)
{
  double q = m_delta / r;
  m_destPoly.push_back(
    FPoint(m_srcPoly[j].X + (m_normals[k].X + m_normals[j].X) * q,
...
{
  double a = std::atan2(
    m_sinA, m_normals[k].X * m_normals[j].X + m_normals[k].Y * m_normals[j].Y);
  int steps = (int) Round(m_StepsPerRad * std::fabs(a));

  double X = m_normals[k].X, Y = m_normals[k].Y, X2;
  for (int i = 0; i < steps; ++i) {
    m_destPoly.push_back(
      FPoint(m_srcPoly[j].X + X * m_delta, m_srcPoly[j].Y + Y * m_delta));
    X2 = X;
    X = X * m_cos - m_sin * Y;
    Y = X2 * m_sin + Y * m_cos;
  }
...
double
DistanceFromLineSqrd(const FPoint& pt, const FPoint& ln1, const FPoint& ln2)
{
...
void MinkowskiSum(const Path& poly,
                  const Path& path,
                  Paths&      solution,
                  bool        isClosed)
{
...
void MinkowskiDiff(const Path& poly,
                   const Path& path,
                   Paths&      solution,
                   bool        isClosed)
{
...
void AddPolyNodeToPolygons(const PolyNode& polynode,
                           NodeType        nodetype,
                           Paths&          paths)
{
  bool match = true;
...
void OffsetPaths(const Paths& in_polys,
                 Paths&       out_polys,
                 double       delta,
                 JoinType     jointype,
                 EndType_     endtype,
