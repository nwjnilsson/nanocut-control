...
bool Geometry::points_match(double_point_t p1, double_point_t p2)
{
  ...
  if (this->between(p1.x, p2.x - tolorance, p2.x + tolorance) &&
      this->between(p1.y, p2.y - tolorance, p2.y + tolorance)) {
    return true;
  }
  else {
    return false;
...
nlohmann::json Geometry::arc_to_line_segments(double cx,
                                              double cy,
                                              double r,
                                              double start_angle,
                                              double end_angle,
  ...
  for (nlohmann::json::const_iterator it = geometry_stack.begin();
    ...
    if (it.value()["type"] == "line") {
      ...
      nlohmann::json line_segments =
        this->arc_to_line_segments(it.value()["center"]["x"],
                                   it.value()["center"]["y"],
                                   it.value()["radius"],
      ...
      nlohmann::json line_segments =
        this->circle_to_line_segments(it.value()["center"]["x"],
                                      it.value()["center"]["y"],
                                      it.value()["radius"]);
      for (nlohmann::json::iterator sub_it = line_segments.begin();
           sub_it != line_segments.end();
...
nlohmann::json Geometry::get_extents(const nlohmann::json& geometry_stack)
{
...
double_point_t Geometry::rotate_point(double_point_t center,
...
nlohmann::json Geometry::offset(const nlohmann::json& path, double offset)
{
  ...
  */
  ...
  // this->RamerDouglasPeucker(pointList, 0.0005 * scale, pointListOut);
...
std::vector<double_point_t>
Geometry::simplify(const std::vector<double_point_t> points, double smoothing)
{
  double                 scale = 100.0f;
  vector<Point>          pointList;
  vector<Point>          pointListOut; // List after simplification
...
double Geometry::measure_arc_circumference(double start_angle,
                                           double end_angle,
                                           double radius)
{
  double angle =
...
double_line_t Geometry::create_polar_line(double_point_t start_point,
                                          double         angle,
                                          double         length)
{
  double_line_t ret;
  ...
  double_point_t rotated_endpoint =
    this->rotate_point(ret.start, ret.end, angle * -1);
  ret.end.x = rotated_endpoint.x;
  ret.end.y = rotated_endpoint.y;
  return ret;
}
double_point_t Geometry::three_point_circle_center(double_point_t p1,
                                                   double_point_t p2,
                                                   double_point_t p3)
{
...
bool Geometry::line_intersects_with_circle(double_line_t  l,
                                           double_point_t center,
                                           double         radius)
{
...
bool Geometry::point_is_inside_polygon(const nlohmann::json& polygon,
...
bool Geometry::polygon_is_inside_polygon(const nlohmann::json& polygon1,
                                         const nlohmann::json& polygon2)
{
  ...
  for (nlohmann::json::const_iterator it = polygon1.begin();
    ...
    if (point_is_inside_polygon(polygon2, point) == false)
      return false;
  }
  return true;
}
