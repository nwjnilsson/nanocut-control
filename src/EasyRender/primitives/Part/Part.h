...
public:
  ...
  Part(std::string name, std::vector<path_t> p)
  {
    ...
    this->control.smoothing = SCALE(0.25f);
    ...
    this->last_control.smoothing = SCALE(0.25f);
  ...
  std::vector<std::vector<double_point_t>>
       OffsetPath(std::vector<double_point_t> path, double offset);
  ...
  double          PerpendicularDistance(const double_point_t& pt,
                                        const double_point_t& lineStart,
                                        const double_point_t& lineEnd);
  void            Simplify(const std::vector<double_point_t>& pointList,
                           std::vector<double_point_t>&       out,
                           double                             epsilon);
  double_point_t* GetClosestPoint(size_t*                      index,
                                  double_point_t               point,
                                  std::vector<double_point_t>* points);
  bool            CreateToolpathLeads(std::vector<double_point_t>* tpath,
                                      double                       length,
