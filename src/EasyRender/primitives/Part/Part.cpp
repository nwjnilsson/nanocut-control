...
std::string EasyPrimitive::Part::get_type_name() { return "part"; }
    ...
    if (this->control.mouse_mode == 0) {
      ...
      for (size_t x = 0; x < this->paths.size(); x++) {
        for (size_t i = 1; i < this->paths[x].built_points.size(); i++) {
          if (g.line_intersects_with_circle(
                { { this->paths[x].built_points.at(i - 1).x,
                    this->paths[x].built_points.at(i - 1).y },
                  { this->paths[x].built_points.at(i).x,
                    this->paths[x].built_points.at(i).y } },
                { mpos_x, mpos_y },
                this->properties->mouse_over_padding /
        ...
        if (this->paths[x].is_closed == true) {
          if (g.line_intersects_with_circle(
                { { this->paths[x].built_points.at(0).x,
                    this->paths[x].built_points.at(0).y },
                  { this->paths[x]
                      .built_points.at(this->paths[x].built_points.size() - 1)
                      .x,
                    this->paths[x]
                      .built_points.at(this->paths[x].built_points.size() - 1)
                      .y } },
      ...
      for (size_t x = 0; x < this->paths.size(); x++) {
        if (this->paths[x].is_inside_contour == false) {
          if (this->CheckIfPointIsInsidePath(this->paths[x].built_points,
                                             { mpos_x, mpos_y })) {
            path_index = x;
...
EasyPrimitive::Part::OffsetPath(std::vector<double_point_t> path, double offset)
{
  double                                   scale = 100.0f;
  std::vector<std::vector<double_point_t>> ret;
  ClipperLib::Path                         subj;
  ClipperLib::Paths                        solution;
  for (std::vector<double_point_t>::iterator it = path.begin();
       it != path.end();
       ++it) {
...
double
EasyPrimitive::Part::PerpendicularDistance(const double_point_t& pt,
                                           const double_point_t& lineStart,
                                           const double_point_t& lineEnd)
{
...
void EasyPrimitive::Part::Simplify(const std::vector<double_point_t>& pointList,
                                   std::vector<double_point_t>&       out,
                                   double                             epsilon)
{
  if (pointList.size() < 2) {
  ...
  for (size_t i = 1; i < end; i++) {
    double d =
      this->PerpendicularDistance(pointList[i], pointList[0], pointList[end]);
    if (d > dmax) {
      index = i;
      dmax = d;
    }
  ...
  if (dmax > epsilon) {
    ...
    std::vector<double_point_t> firstLine(pointList.begin(),
                                          pointList.begin() + index + 1);
    std::vector<double_point_t> lastLine(pointList.begin() + index,
                                         pointList.end());
...
void EasyPrimitive::Part::render()
{
  if (!(this->last_control == this->control)) {
    ...
    for (std::vector<path_t>::iterator it = this->paths.begin();
         it != this->paths.end();
         ++it) {
      ...
      try {
        ...
        for (size_t i = 0; i < simplified.size(); i++) {
          double_point_t rotated =
            g.rotate_point({ 0, 0 }, simplified[i], this->control.angle);
          it->built_points.push_back(
            { (rotated.x + this->control.offset.x) * this->control.scale,
              (rotated.y + this->control.offset.y) * this->control.scale,
              (rotated.z + this->control.offset.z) * this->control.scale });
          this->number_of_verticies++;
        }
        if (it->toolpath_visible == true) {
          if (it->is_closed == true) {
            if (it->is_inside_contour == true) {
              std::vector<std::vector<double_point_t>> tpaths =
                this->OffsetPath(it->built_points,
                                 -(double) fabs(it->toolpath_offset));
              for (size_t x = 0; x < tpaths.size(); x++) {
                size_t count = 0;
                while (this->CreateToolpathLeads(&tpaths[x],
                                                 this->control.lead_in_length,
                                                 tpaths[x].front(),
                                                 -1) == false &&
                       count < tpaths[x].size()) {
                  if (tpaths[x].size() > 2) {
        ...
        LOG_F(ERROR,
              "(EasyPrimitive::Part::render) Exception: %s, setting visability "
  ...
  for (std::vector<path_t>::iterator it = this->paths.begin();
    ...
    try {
      for (int i = 0; i < it->built_points.size(); i++) {
        glVertex3f(
          it->built_points[i].x, it->built_points[i].y, it->built_points[i].z);
      }
    }
    catch (std::exception& e) {
      LOG_F(ERROR,
  ...
  for (int x = 0; x < this->tool_paths.size(); x++) {
    ...
    for (int i = 0; i < this->tool_paths[x].size(); i++) {
      glVertex3f(this->tool_paths[x][i].x,
                 this->tool_paths[x][i].y,
                 this->tool_paths[x][i].z);
    }
    glEnd();
  }
  glLineWidth(1);
...
void           EasyPrimitive::Part::destroy() { delete this->properties; }
nlohmann::json EasyPrimitive::Part::serialize()
{
  ...
  for (std::vector<path_t>::iterator it = this->paths.begin();
    ...
    for (int i = 0; i < it->points.size(); i++) {
      path["points"].push_back({ { "x", it->points[i].x },
                                 { "y", it->points[i].y },
                                 { "z", it->points[i].z } });
    }
...
void EasyPrimitive::Part::GetBoundingBox(double_point_t* bbox_min,
                                         double_point_t* bbox_max)
{
  ...
  for (std::vector<path_t>::iterator it = this->paths.begin();
       it != this->paths.end();
       ++it) {
    for (std::vector<double_point_t>::iterator p = it->built_points.begin();
...
bool EasyPrimitive::Part::CheckIfPointIsInsidePath(
...
bool EasyPrimitive::Part::CheckIfPathIsInsidePath(
...
std::vector<std::vector<double_point_t>>
EasyPrimitive::Part::GetOrderedToolpaths()
{
  Geometry                                 g;
  ...
  if (toolpaths.size() > 0) {
    ...
    while (toolpaths.size() > 0) {
      ...
      if (winner_index == -1) {
        LOG_F(WARNING,
              "(EasyPrimitive::Part::GetOrderedToolpaths) Discarded empty "
              "toolpath!");
      }
      else {
...
double_point_t* EasyPrimitive::Part::GetClosestPoint(
  size_t* index, double_point_t point, std::vector<double_point_t>* points)
{
  Geometry g;
...
bool EasyPrimitive::Part::CreateToolpathLeads(
  std::vector<double_point_t>* tpath,
  double                       length,
  ...
  int                          direction)
{
  if (length == 0)
    return true;
  Geometry                                 g;
  std::vector<std::vector<double_point_t>> lead_offset =
    this->OffsetPath(*tpath, (double) fabs(length) * (double) direction);
  for (size_t x = 0; x < lead_offset.size(); x++) {
    double_point_t* point =
      this->GetClosestPoint(NULL, position, &lead_offset[x]);
    if (point != NULL) {
