...
void EasyRender::mouse_button_callback(GLFWwindow* window,
                                       int         button,
                                       int         action,
                                       int         mods)
{
  ...
  if (self != NULL) {
    ...
    if (!self->imgui_io->WantCaptureKeyboard ||
      ...
      for (it = self->Primitive_stack.end();
           it != self->Primitive_stack.begin();) {
        --it;
        if ((*it)->properties->view == self->CurrentView) {
          if ((*it)->properties->visible == true &&
              (*it)->properties->mouse_over == true) {
            if ((*it)->properties->mouse_callback != NULL) {
              double_point_t matrix_mouse = m;
              matrix_mouse.x = (matrix_mouse.x - (*it)->properties->offset[0]) /
                               (*it)->properties->scale;
              matrix_mouse.y = (matrix_mouse.y - (*it)->properties->offset[1]) /
                               (*it)->properties->scale;
...
void EasyRender::scroll_callback(GLFWwindow* window,
                                 double      xoffset,
                                 double      yoffset)
{
  EasyRender* self =
...
void EasyRender::cursor_position_callback(GLFWwindow* window,
                                          double      xpos,
                                          double      ypos)
{
  EasyRender* self =
...
EasyRender::EasyRenderGui*
EasyRender::PushGui(bool v, void (*c)(void* p), void* s)
{
  EasyRender::EasyRenderGui* g = new EasyRender::EasyRenderGui;
  g->visible = v;
  g->callback = NULL;
                           ...
                           void (*callback)(const nlohmann::json&))
{
  ...
  if (type == "window_resize" && this->Window != NULL) {
    ...
    EasyRender::window_size_callback(
      this->Window, this->WindowSize[0], this->WindowSize[1]);
  }
}

void EasyRender::SetWindowTitle(std::string w) { this->WindowTitle = w; }
...
void EasyRender::SetShowCursor(bool s) { this->ShowCursor = s; }

void EasyRender::SetAutoMaximize(bool m) { this->AutoMaximize = m; }
...
void EasyRender::SetGuiStyle(std::string s) { this->GuiStyle = s; }
...
void EasyRender::SetClearColor(float r, float g, float b)
{
  ...
  // LOG_F(INFO, "Set Clear Color (%.4f, %.4f, %.4f)", this->ClearColor[0],
  // this->ClearColor[1], this->ClearColor[2]);
}

void EasyRender::SetShowFPS(bool show_fps) { this->ShowFPS = show_fps; }
...
void EasyRender::SetColorByName(float* c, std::string color)
{
  if (color == "white") {
    ...
    LOG_F(WARNING,
          "(EasyRender::SetColorByName) Color \"%s\" not found, defaulting to "
          "white",
          color.c_str());
    c[0] = 255;
    c[1] = 255;
...
void EasyRender::SetCurrentView(std::string v) { this->CurrentView = v; }
...
unsigned long EasyRender::Millis()
{
  ...
  unsigned long m =
    (unsigned long) std::chrono::duration_cast<std::chrono::milliseconds>(
      end - EasyRenderProgramStartTime)
      .count();
  return m;
}
...
double_point_t EasyRender::GetWindowMousePosition()
{
  ...
  mouseX -= 2;
  mouseY -= 4;
  return { mouseX - (this->WindowSize[0] / 2.0f),
           (this->WindowSize[1] - mouseY) - (this->WindowSize[1] / 2.0f) };
}
...
std::string EasyRender::GetCurrentView() { return this->CurrentView; }
...
void EasyRender::DumpJsonToFile(std::string filename, const nlohmann::json& j)
{
  try {
...
bool EasyRender::Init(int argc, char** argv)
{
  ...
  this->Window = glfwCreateWindow(this->WindowSize[0],
                                  this->WindowSize[1],
...
bool EasyRender::Poll(bool should_quit)
{
  ...
  glClearColor(
    this->ClearColor[0], this->ClearColor[1], this->ClearColor[2], 255);
  /* IMGUI */
  ImGui_ImplOpenGL2_NewFrame();
  ImGui_ImplGlfw_NewFrame();
  ImGui::NewFrame();
  for (size_t x = 0; x < this->gui_stack.size(); x++) {
    if (this->gui_stack[x]->visible == true &&
        this->gui_stack[x]->view == this->CurrentView) {
      if (this->gui_stack[x]->self_pointer == NULL) {
        ...
        if (this->gui_stack[x]->callback_with_self != NULL) {
          this->gui_stack[x]->callback_with_self(
            this->gui_stack[x]->self_pointer);
        }
      }
    }
  }
  ...
  glLoadIdentity();
  glOrtho(-(GLdouble) this->WindowSize[0] / 2,
          (GLdouble) this->WindowSize[0] / 2,
          -(GLdouble) this->WindowSize[1] / 2,
          (GLdouble) this->WindowSize[1] / 2,
          -1,
          1);
  ...
  sort(
    Primitive_stack.begin(), Primitive_stack.end(), [](auto* lhs, auto* rhs) {
  ...
  for (size_t x = 0; x < this->Primitive_stack.size(); x++) {
    if (this->Primitive_stack[x]->properties->visible == true &&
        this->Primitive_stack[x]->properties->view == this->CurrentView) {
      Primitive_stack[x]->render();
      if ((!this->imgui_io->WantCaptureKeyboard ||
           !this->imgui_io->WantCaptureMouse) &&
          ignore_next_mouse_events == false) {
        Primitive_stack[x]->process_mouse(window_mouse_pos.x,
                                          window_mouse_pos.y);
      }
    }
  }
  ...
  for (size_t x = 0; x < this->timer_stack.size(); x++) {
    if (this->timer_stack.at(x)->view == this->CurrentView) {
      if ((this->Millis() - this->timer_stack.at(x)->timestamp) >
          this->timer_stack.at(x)->interval) {
        ...
        if (this->timer_stack[x]->self_pointer == NULL) {
          ...
          if (this->timer_stack[x]->callback_with_self != NULL) {
            if (this->timer_stack[x]->callback_with_self(
                  this->timer_stack[x]->self_pointer) == false) // Don't repeat
            {
              delete this->timer_stack[x];
              this->timer_stack.erase(this->timer_stack.begin() + x);
            }
  ...
  if (this->ShowFPS == true) {
    if (this->FPS_Label == NULL) {
      ...
      this->FPS_Label =
        this->PushPrimitive(new EasyPrimitive::Text({ 0, 0 }, "0", 30));
