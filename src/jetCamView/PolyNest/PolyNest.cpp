                                        ...
                                        PolyPoint* bbox_max)
{
  ...
  for (std::vector<PolyGon>::iterator it = this->built_polygons.begin();
       it != this->built_polygons.end();
       ++it) {
    ...
    for (size_t i = 0; i < it->vertices.size(); i++) {
      bbox_min->x = std::min(it->vertices[i].X, bbox_min->x);
      bbox_min->y = std::min(it->vertices[i].Y, bbox_min->y);
      bbox_max->x = std::max(it->vertices[i].X, bbox_max->x);
      bbox_max->y = std::max(it->vertices[i].Y, bbox_max->y);
    }
  }
  // LOG_F(INFO,
  //       "Computed bounding box: [%f, %f], [%f, %f]",
  //       bbox_min->x,
  //       bbox_min->y,
  //       bbox_max->x,
  //       bbox_max->y);
...
void PolyNest::PolyPart::Build()
{
  ...
  for (size_t x = 0; x < this->polygons.size(); x++) {
    ...
    for (size_t i = 0; i < this->polygons[x].vertices.size(); i++) {
      PolyPoint point =
        this->RotatePoint(PolyPoint(this->polygons[x].vertices[i].X,
                                    this->polygons[x].vertices[i].Y),
                          *this->angle);
      point.x += *this->offset_x;
      point.y += *this->offset_y;
      p.vertices << ClipperLib::FPoint(point.x, point.y);
    }
...
{
  for (std::vector<PolyPoint>::iterator it = path1.begin(); it != path1.end();
       ++it) {
...
bool PolyNest::PolyNest::CheckIfPolyPartTouchesAnyPlacedPolyPart(PolyPart p)
{
  for (size_t x = 0; x < this->placed_parts.size(); x++) {
    ...
    c.AddPath(p.built_polygons.front().vertices, ClipperLib::ptSubject, true);
    c.AddPath(this->placed_parts[x].built_polygons.back().vertices,
              ClipperLib::ptClip,
              true);
...
bool PolyNest::PolyNest::CheckIfPolyPartIsInsideExtents(PolyPart  p,
                                                        ...
                                                        PolyPoint max_point)
{
  if (p.bbox_min.x >= min_point.x && p.bbox_max.x <= max_point.x &&
      p.bbox_min.y >= min_point.y && p.bbox_max.y <= max_point.y) {
    return true;
  }
  else {
    return false;
                              ...
                              bool*                               visible)
{
  ...
  for (size_t x = 0; x < p.size(); x++) {
    if (p[x].size() > 2) {
      ...
      if (solution.size() > 0) {
        polygon.vertices = solution.front();
      }
      else {
        polygon.vertices = subj;
        LOG_F(
          ERROR,
          "Zero solutions on offset! Perhaps some pre-processing is needed?");
        continue;
      }
      if (this->MeasureDistanceBetweenPoints(
            PolyPoint(polygon.vertices.front().X, polygon.vertices.front().Y),
            PolyPoint(polygon.vertices.back().X, polygon.vertices.back().Y)) <
          this->closed_tolerance) {
...
{
  double px =
    min_extents.x - part.bbox_min.x + DEFAULT_LEAD_IN + DEFAULT_KERF_WIDTH;
  double py =
    min_extents.y - part.bbox_min.y + DEFAULT_LEAD_IN + DEFAULT_KERF_WIDTH;
  return { px, py };
}
...
{
  std::sort(this->unplaced_parts.begin(),
            this->unplaced_parts.end(),
            [](const PolyPart& lhs, const PolyPart& rhs) {
              return (lhs.bbox_max.x - lhs.bbox_min.x) *
                       (lhs.bbox_max.y - lhs.bbox_min.y) >
                     (rhs.bbox_max.x - rhs.bbox_min.x) *
                       (rhs.bbox_max.y - rhs.bbox_min.y);
            });
  if (this->unplaced_parts.size() > 0 &&
      *this->unplaced_parts.front().offset_x == 0 &&
      *this->unplaced_parts.front().offset_y == 0) {
    auto& part     = this->unplaced_parts.front();
    *part.offset_x = GetDefaultOffsetXY(part).x;
    *part.offset_y = GetDefaultOffsetXY(part).y;
    // LOG_F(INFO, "initial offset (%.3f, %.3f) (rotation: %.3f)",
    // *part.offset_x, *part.offset_y, *part.angle);
  }
...
bool PolyNest::PolyNest::PlaceUnplacedPolyPartsTick(void* p)
{
  ...
  if (self != NULL) {
    while (not self->unplaced_parts.empty()) {
      const double bbox_x_half = (self->unplaced_parts.front().bbox_max.x -
                                  self->unplaced_parts.front().bbox_min.x) /
                                 2;
      const double bbox_y_half = (self->unplaced_parts.front().bbox_max.y -
                                  self->unplaced_parts.front().bbox_min.y) /
                                 2;
        ...
        for (size_t x = 0; x < (360.0 / self->rotation_increments); x++) {
          if (self->CheckIfPolyPartTouchesAnyPlacedPolyPart(part) == false &&
              self->CheckIfPolyPartIsInsideExtents(
                part, self->min_extents, self->max_extents) == true) {
