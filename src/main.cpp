...
void log_uptime()
{
  ...
  LOG_F(INFO,
        ...
        minutes,
        seconds);
  nlohmann::json uptime_json = globals->renderer->ParseJsonFromFile(
    globals->renderer->GetConfigDirectory() + "uptime.json");
  if (uptime_json != NULL) {
    try {
    ...
    globals->renderer->DumpJsonToFile(
      globals->renderer->GetConfigDirectory() + "uptime.json", uptime);
  }
  else {
...
int main(int argc, char** argv)
{
  ...
  globals->renderer->SetGuiIniFileName(globals->renderer->GetConfigDirectory() +
                                       "gui.ini");
  globals->renderer->SetGuiLogFileName(globals->renderer->GetConfigDirectory() +
                                       "gui.log");
  globals->renderer->SetMainLogFileName(
    globals->renderer->GetConfigDirectory() + "NanoCut.log");
  // globals->renderer->SetShowFPS(true);
  globals->renderer->SetWindowSize(
  ...
  std::string config_font_path =
    globals->renderer->GetConfigDirectory() + "fonts/Cabin/Cabin-SemiBold.ttf";
