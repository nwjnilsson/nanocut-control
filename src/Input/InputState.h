...
/**
 * InputState manages the current state of all input devices.
 * This class provides efficient O(1) lookups for key/button press states
 * and maintains current mouse position.
 * 
 * Replaces the scattered mouse position tracking and JSON-based
...
public:
    ...
    const Point2d& getPreviousMousePosition() const { return m_previous_mouse_position; }
    ...
    double getMouseDeltaX() const { return m_mouse_position.x - m_previous_mouse_position.x; }
    double getMouseDeltaY() const { return m_mouse_position.y - m_previous_mouse_position.y; }
    ...
    bool isCtrlShiftPressed() const { return isCtrlPressed() && isShiftPressed(); }
    bool isCtrlAltPressed() const { return isCtrlPressed() && isAltPressed(); }
    
    // Mouse button shortcuts
    bool isLeftMousePressed() const { return isMouseButtonPressed(GLFW_MOUSE_BUTTON_LEFT); }
    bool isRightMousePressed() const { return isMouseButtonPressed(GLFW_MOUSE_BUTTON_RIGHT); }
    bool isMiddleMousePressed() const { return isMouseButtonPressed(GLFW_MOUSE_BUTTON_MIDDLE); }
    ...
    size_t getNumPressedMouseButtons() const { return m_pressed_mouse_buttons.size(); }
