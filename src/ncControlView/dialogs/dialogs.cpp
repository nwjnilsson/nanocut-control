...
std::string                ask_text;
std::function<void()>      ask_window_yes_callback;
std::function<void()>      ask_window_no_callback;
...
void dialogs_preferences()
{
  ImGui::Begin("Preferences",
               &preferences_window_handle->visible,
               ImGuiWindowFlags_AlwaysAutoResize);
  ImGui::ColorEdit3("Background Color",
                    globals->nc_control_view->preferences.background_color);
  ImGui::ColorEdit3("Machine Plane Color",
                    globals->nc_control_view->preferences.machine_plane_color);
  ImGui::ColorEdit3("Cuttable Plane Color",
                    globals->nc_control_view->preferences.cuttable_plane_color);
  ImGui::InputInt2("Default Window Size",
                   globals->nc_control_view->preferences.window_size);
  ImGui::SameLine();
  if (ImGui::Button("<= Current Size")) {
    globals->nc_control_view->preferences.window_size[0] =
      (int) globals->renderer->GetWindowSize().x;
    globals->nc_control_view->preferences.window_size[1] =
      (int) globals->renderer->GetWindowSize().y;
  }
  ImGui::Spacing();
  if (ImGui::Button("OK")) {
    globals->renderer->SetClearColor(
      globals->nc_control_view->preferences.background_color[0] * 255.0f,
      globals->nc_control_view->preferences.background_color[1] * 255.0f,
      globals->nc_control_view->preferences.background_color[2] * 255.0f);
    ...
    preferences["background_color"]["r"] =
      globals->nc_control_view->preferences.background_color[0];
    preferences["background_color"]["g"] =
      globals->nc_control_view->preferences.background_color[1];
    preferences["background_color"]["b"] =
      globals->nc_control_view->preferences.background_color[2];

    preferences["machine_plane_color"]["r"] =
      globals->nc_control_view->preferences.machine_plane_color[0];
    preferences["machine_plane_color"]["g"] =
      globals->nc_control_view->preferences.machine_plane_color[1];
    preferences["machine_plane_color"]["b"] =
      globals->nc_control_view->preferences.machine_plane_color[2];

    preferences["cuttable_plane_color"]["r"] =
      globals->nc_control_view->preferences.cuttable_plane_color[0];
    preferences["cuttable_plane_color"]["g"] =
      globals->nc_control_view->preferences.cuttable_plane_color[1];
    preferences["cuttable_plane_color"]["b"] =
      globals->nc_control_view->preferences.cuttable_plane_color[2];
...
{
  static ncControlView::machine_parameters_data_t temp_parameters =
    globals->nc_control_view->machine_parameters;
  ImGui::Begin("Machine Parameters",
               &machine_parameters_window_handle->visible,
               ImGuiWindowFlags_AlwaysAutoResize);
  ImGui::Separator();
  ImGui::Text(
  ...
  ImGui::InputFloat3("Machine Extents (X, Y, Z).",
                     temp_parameters.machine_extents);
  ...
  ImGui::InputFloat4("Cutting Extents (X1, Y1, X2, Y2)",
                     temp_parameters.cutting_extents);
  ImGui::Separator();
...
void dialogs_set_progress_value(float p) { progress = p; }
void dialogs_progress_window()
{
  ImGui::Begin("Progress",
               &progress_window_handle->visible,
               ImGuiWindowFlags_AlwaysAutoResize);
...
void dialogs_show_info_window(bool s) { info_window_handle->visible = s; }
...
{
  ImGui::Begin(
    "Info", &info_window_handle->visible, ImGuiWindowFlags_AlwaysAutoResize);
...
void dialogs_controller_offline_window()
{
  ImGui::Begin("Controller Offline",
               &controller_offline_window_handle->visible,
               ImGuiWindowFlags_AlwaysAutoResize);
  ImGui::Text(
...
void dialogs_controller_alarm_window()
{
  ImGui::Begin("Controller Alarm",
               &controller_alarm_window_handle->visible,
               ImGuiWindowFlags_AlwaysAutoResize);
...
void dialogs_controller_homing_window()
{
  ImGui::Begin("Home Machine",
               &controller_homing_window_handle->visible,
               ImGuiWindowFlags_AlwaysAutoResize);
...
{
  ask_text = a;
  ask_window_yes_callback = yes_callback;
  ask_window_no_callback = no_callback;
...
{
  ImGui::Begin(
    "Question", &ask_window_handle->visible, ImGuiWindowFlags_AlwaysAutoResize);
  ImGui::Text("%s", ask_text.c_str());
  if (ImGui::Button("Yes")) {
    if (ask_window_yes_callback)
      ask_window_yes_callback();
    ask_window_handle->visible = false;
  }
  ImGui::SameLine();
  if (ImGui::Button("No")) {
    if (ask_window_no_callback)
      ask_window_no_callback();
    ask_window_handle->visible = false;
  }
  ImGui::End();
}

void dialogs_show_thc_window(bool s) { thc_window_handle->visible = s; }
...
void dialogs_init()
{
  ...
  preferences_window_handle =
    globals->renderer->PushGui(false, dialogs_preferences);
  machine_parameters_window_handle =
    globals->renderer->PushGui(false, dialogs_machine_parameters);
  progress_window_handle =
    globals->renderer->PushGui(false, dialogs_progress_window);
  info_window_handle = globals->renderer->PushGui(false, dialogs_info_window);
  controller_offline_window_handle =
    globals->renderer->PushGui(false, dialogs_controller_offline_window);
  controller_alarm_window_handle =
    globals->renderer->PushGui(false, dialogs_controller_alarm_window);
  controller_homing_window_handle =
    globals->renderer->PushGui(false, dialogs_controller_homing_window);
