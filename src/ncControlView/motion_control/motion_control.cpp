...
void motion_controller_probe()
{
  motion_controller_send_crc32(
    "G38.3Z" +
    std::to_string(
      globals->nc_control_view->machine_parameters.machine_extents[2] -
      globals->nc_control_view->machine_parameters.homing_pull_off) +
    "F" +
    std::to_string(
      globals->nc_control_view->machine_parameters.z_probe_feedrate));
}

bool arc_okay_expire_timer()
{
  if (abort_pending == true)
    return false; // Pending abort must cancel this timer otherwise aborts would
                  // be ignored in arc retry loop!
  /*
      if this function is called, torch fired but controller never seen an arc
     okay signal after 3 seconds...
  */
  if (arc_okay_callback != NULL) // Only run this code if the arc_okay_callback
                                 // was not processed, meaning the timr event
                                 // fired and the arc never transfered after n
                                 // seconds
  {
  ...
  if (gcode_stack.size() > 0) {
    ...
    if (line.find("fire_torch") != std::string::npos) {
      ...
      LOG_F(INFO,
            ...
            "fires in %.4f ms!",
            (3 + (double) callback_args["pierce_delay"]) * 1000);
      globals->renderer->PushTimer(
        (3 + (double) callback_args["pierce_delay"]) * 1000,
        &arc_okay_expire_timer);
...
/*          end callbacks           */
nlohmann::json motion_controller_get_dro() { return dro_data; }
...
void motion_controller_log_runtime()
{
  ...
  LOG_F(INFO,
        ...
        minutes,
        seconds);
  nlohmann::json runtime_json = globals->renderer->ParseJsonFromFile(
    globals->renderer->GetConfigDirectory() + "runtime.json");
  if (runtime_json != NULL) {
    try {
      runtime["total_milliseconds"] =
        (unsigned long) runtime_json["total_milliseconds"] + m;
    }
    catch (...) {
  ...
  if (controller_ready == true) {
    if (line.find("{") != std::string::npos) {
      try {
        ...
        if (dro_data["ARC_OK"] == false) // Logic is inverted. Bool goes low
                                         // when controller has arc_ok signal
        {
      ...
      if (globals->nc_control_view->machine_parameters.homing_enabled ==
          false) {
        LOG_F(
          WARNING,
      ...
      if (last_checksum_error > 0 &&
          (EasyRender::Millis() - last_checksum_error) < 100) {
        dialogs_set_info_value("Program aborted due to multiple communication "
...
void motion_controller_save_machine_parameters()
{
  ...
  preferences["max_vel"]["x"] =
    globals->nc_control_view->machine_parameters.max_vel[0];
  preferences["max_vel"]["y"] =
    globals->nc_control_view->machine_parameters.max_vel[1];
  preferences["max_vel"]["z"] =
    globals->nc_control_view->machine_parameters.max_vel[2];
  ...
  preferences["arc_stabilization_time"] =
    globals->nc_control_view->machine_parameters.arc_stabilization_time;
  ...
  preferences["axis_invert"]["x"] =
    globals->nc_control_view->machine_parameters.axis_invert[0];
  preferences["axis_invert"]["y1"] =
    globals->nc_control_view->machine_parameters.axis_invert[1];
  preferences["axis_invert"]["y2"] =
    globals->nc_control_view->machine_parameters.axis_invert[2];
  preferences["axis_invert"]["z"] =
    globals->nc_control_view->machine_parameters.axis_invert[3];
  preferences["soft_limits_enabled"] =
    globals->nc_control_view->machine_parameters.soft_limits_enabled;
  preferences["homing_enabled"] =
    globals->nc_control_view->machine_parameters.homing_enabled;
  preferences["homing_dir_invert"]["x"] =
    globals->nc_control_view->machine_parameters.homing_dir_invert[0];
  preferences["homing_dir_invert"]["y"] =
    globals->nc_control_view->machine_parameters.homing_dir_invert[1];
  preferences["homing_dir_invert"]["z"] =
    globals->nc_control_view->machine_parameters.homing_dir_invert[2];
  preferences["homing_feed"] =
    globals->nc_control_view->machine_parameters.homing_feed;
  preferences["homing_seek"] =
    globals->nc_control_view->machine_parameters.homing_seek;
  ...
  preferences["invert_limit_pins"] =
    globals->nc_control_view->machine_parameters.invert_limit_pins;
  preferences["invert_probe_pin"] =
    globals->nc_control_view->machine_parameters.invert_probe_pin;
  preferences["invert_step_enable"] =
    globals->nc_control_view->machine_parameters.invert_step_enable;
  ...
  globals->nc_control_view->machine_plane->width =
    globals->nc_control_view->machine_parameters.machine_extents[0];
  globals->nc_control_view->machine_plane->height =
    globals->nc_control_view->machine_parameters.machine_extents[1];
  globals->nc_control_view->cuttable_plane->bottom_left.x =
    globals->nc_control_view->machine_parameters.cutting_extents[0];
  globals->nc_control_view->cuttable_plane->bottom_left.y =
    globals->nc_control_view->machine_parameters.cutting_extents[1];
  globals->nc_control_view->cuttable_plane->width =
    (globals->nc_control_view->machine_parameters.machine_extents[0] +
     globals->nc_control_view->machine_parameters.cutting_extents[2]) -
    globals->nc_control_view->machine_parameters.cutting_extents[0];
  globals->nc_control_view->cuttable_plane->height =
    (globals->nc_control_view->machine_parameters.machine_extents[1] +
     globals->nc_control_view->machine_parameters.cutting_extents[3]) -
    globals->nc_control_view->machine_parameters.cutting_extents[1];
  try {
    std::ofstream out(globals->renderer->GetConfigDirectory() +
...
void motion_controller_write_parameters_to_controller()
{
                                      ...
                                      // flash
  {
    ...
    if (globals->nc_control_view->machine_parameters.axis_invert[0])
      dir_invert_mask |= 0b00000001;
    if (globals->nc_control_view->machine_parameters.axis_invert[1])
      dir_invert_mask |= 0b00000010;
    if (globals->nc_control_view->machine_parameters.axis_invert[3])
      dir_invert_mask |= 0b00000100;
    if (globals->nc_control_view->machine_parameters.axis_invert[2])
      dir_invert_mask |= 0b00001000;

    uint8_t homing_dir_invert_mask = 0b00000000;
    if (globals->nc_control_view->machine_parameters.homing_dir_invert[0])
      homing_dir_invert_mask |= 0b00000001;
    if (globals->nc_control_view->machine_parameters.homing_dir_invert[1])
...
bool motion_control_status_timer()
{
  ...
  if (controller_ready == true) {
    if (dro_data.contains("STATUS")) {
      if (motion_sync_callback != NULL &&
          (bool) dro_data["IN_MOTION"] == false &&
          (EasyRender::Millis() - program_run_time) > 500) {
...
void motion_controller_set_needs_homed(bool h) { needs_homed = h; }
...
void motion_control_tick()
{
  ...
  if (needs_homed == true &&
      globals->nc_control_view->machine_parameters.homing_enabled == true &&
      motion_controller.is_connected == true && controller_ready == true &&
      dialogs_controller_alarm_window_visible() == false) {
  ...
  try {
    if (torch_on == true &&
        (EasyRender::Millis() - arc_okay_timer) >
          (globals->nc_control_view->machine_parameters.arc_stabilization_time +
           ((float) callback_args["pierce_delay"] * 1000))) {
