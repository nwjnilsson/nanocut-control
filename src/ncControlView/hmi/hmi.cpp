  ...
  for (int x = 0; x < stack->size(); x++) {
    if (stack->at(x)->type == "path") {
      for (int i = 0; i < stack->at(x)->path->points.size(); i++) {
        const float px =
          stack->at(x)->path->points.at(i).x -
          globals->nc_control_view->machine_parameters.work_offset[0];
        const float py =
          stack->at(x)->path->points.at(i).y -
...
static bool check_path_bounds()
{
  ...
  if (bbox_min.x >
        0.0f +
          globals->nc_control_view->machine_parameters.cutting_extents[0] &&
      bbox_min.y >
        0.0f +
          globals->nc_control_view->machine_parameters.cutting_extents[1] &&
      bbox_max.x <
        globals->nc_control_view->machine_parameters.machine_extents[0] +
          globals->nc_control_view->machine_parameters.cutting_extents[2] &&
      bbox_max.y <
        globals->nc_control_view->machine_parameters.machine_extents[1] +
          globals->nc_control_view->machine_parameters.cutting_extents[3]) {
    return true;
  }
  LOG_F(WARNING,
...
void hmi_handle_button(std::string id)
{
  ...
  try {
    ...
    if (dro_data["IN_MOTION"] == false) {
      ...
      //                motion_controller_push_stack("G10 L2 P0 Z" +
      //                std::to_string(globals->nc_control_view->machine_parameters.work_offset[2]));
      //                motion_controller_push_stack("M30");
      //                motion_controller_run_stack();
      //                motion_controller_save_machine_parameters();
      //            }
      ...
      if (bbox_max.x == std::numeric_limits<int>::min() &&
        ...
        globals->pan.y =
          -((globals->nc_control_view->machine_parameters.machine_extents[1]) /
            2.0);
        auto y_pair = minmax(
          globals->renderer->GetWindowSize().y,
          globals->nc_control_view->machine_parameters.machine_extents[1]);
        auto x_pair = minmax(
          globals->renderer->GetWindowSize().x - hmi_backplane_width,
          globals->nc_control_view->machine_parameters.machine_extents[0]);
        ...
        else // Fit X
        {
          ...
          globals->zoom =
            (window_x - (hmi_backplane_width / 2.0)) / (machine_x + window_x);
        }
        globals->pan.x +=
          ((double)
             globals->nc_control_view->machine_parameters.machine_extents[0] /
        ...
        auto x_pair = minmax(
          globals->renderer->GetWindowSize().x - hmi_backplane_width, x_diff);
        if (y_pair.second - y_pair.first < x_pair.second - x_pair.first) {
          LOG_F(INFO, "Fitting to Y");
          globals->zoom =
            ((double) globals->renderer->GetWindowSize().y) / (2 * y_diff);
        }
        else // Fit X
        {
          LOG_F(INFO, "Fitting to X");
          globals->zoom = ((double) globals->renderer->GetWindowSize().x -
                           (hmi_backplane_width / 2)) /
                          (2 * x_diff);
        }
        ...
        while (std::getline(gcode_file, line)) {
          if (count <= (unsigned long) p->properties->data["rapid_line"] + 1) {
            gcode_lines_before_reverse.push_back(line);
                   ...
                   (unsigned long) p->properties->data["rapid_line"] + 1) {
...
{
  const float wco_zoom_x =
    globals->pan.x +
    abs(globals->nc_control_view->machine_parameters.work_offset[0] *
        globals->zoom);
  const float wco_zoom_y =
    globals->pan.y +
    abs(globals->nc_control_view->machine_parameters.work_offset[1] *
        globals->zoom);
  ...
  if (p->type == "path") {
    if (p->properties->id == "gcode" ||
        p->properties->id == "gcode_highlights") {
      p->properties->scale = globals->zoom;
      p->properties->offset[0] = wco_zoom_x;
      p->properties->offset[1] = wco_zoom_y;
    }
...
void hmi_resize_callback(const nlohmann::json& e)
{
  // LOG_F(INFO, "(hmi_resize_callback) %s", e.dump().c_str());
  hmi_backpane->bottom_left.x =
    (globals->renderer->GetWindowSize().x / 2) - hmi_backplane_width;
  ...
  hmi_dro_backpane->bottom_left.y =
    (globals->renderer->GetWindowSize().y / 2) - hmi_dro_backplane_height;
  ...
  dro_group_y += 10;
  dro.feed->position = { (float) dro_group_x + 5,
                         (float) dro_group_y - dro.feed->height + 5 };
  dro.arc_readout->position = {
    (float) dro_group_x + 80, (float) dro_group_y - dro.arc_readout->height + 5
  };
  dro.arc_set->position = { (float) dro_group_x + 150,
                            (float) dro_group_y - dro.arc_set->height + 5 };
  dro.run_time->position = { (float) dro_group_x + 210,
                             (float) dro_group_y - dro.run_time->height + 5 };
  dro_group_y += 55;
  dro.z.label->position = { (float) dro_group_x + 5,
                            (float) dro_group_y - dro.z.label->height };
  dro.z.work_readout->position = { (float) dro_group_x + 5 + 50,
                                   (float) dro_group_y - dro.z.label->height };
  dro.z.absolute_readout->position = {
    (float) dro_group_x + 5 + 220, (float) dro_group_y - dro.z.label->height
  };
  dro.z.divider->bottom_left = { (float) dro_group_x + 5,
                                 (float) dro_group_y - 45 };
  dro.z.divider->width = hmi_backplane_width - 20;
  dro.z.divider->height = 5;
  dro_group_y += 55;
  dro.y.label->position = { (float) dro_group_x + 5,
                            (float) dro_group_y - dro.y.label->height };
  dro.y.work_readout->position = { (float) dro_group_x + 5 + 50,
                                   (float) dro_group_y - dro.y.label->height };
  dro.y.absolute_readout->position = {
    (float) dro_group_x + 5 + 220, (float) dro_group_y - dro.y.label->height
  };
  dro.y.divider->bottom_left = { (float) dro_group_x + 5,
                                 (float) dro_group_y - 45 };
  dro.y.divider->width = hmi_backplane_width - 20;
  dro.y.divider->height = 5;
  dro_group_y += 55;
  dro.x.label->position = { (float) dro_group_x + 5,
                            (float) dro_group_y - dro.x.label->height };
  dro.x.work_readout->position = { (float) dro_group_x + 5 + 50,
                                   (float) dro_group_y - dro.x.label->height };
  dro.x.absolute_readout->position = {
    (float) dro_group_x + 5 + 220, (float) dro_group_y - dro.x.label->height
  };
  dro.x.divider->bottom_left = { (float) dro_group_x + 5,
                                 (float) dro_group_y - 45 };
  ...
  hmi_button_backpane->height =
    globals->renderer->GetWindowSize().y - (hmi_dro_backplane_height + 30);

  double button_group_x = hmi_button_backpane->bottom_left.x;
  double button_group_y =
    hmi_button_backpane->bottom_left.y + hmi_button_backpane->height;
  double button_height =
    (hmi_button_backpane->height - 10) / (double) button_groups.size();
  ...
  for (int x = 0; x < button_groups.size(); x++) {
    ...
    button_groups[x].button_one.object->bottom_left.y =
    ...
    center_x =
      button_groups[x].button_one.object->bottom_left.x + (button_width / 2);
    center_y =
      button_groups[x].button_one.object->bottom_left.y + (button_height / 2);
    button_groups[x].button_one.label->position = {
      (float) center_x - (button_groups[x].button_one.label->width / 2.0f) - 5,
      (float) center_y - (button_groups[x].button_one.label->height) + 5
    };

    button_group_x += button_width;
    button_groups[x].button_two.object->bottom_left.x = button_group_x + 5;
    button_groups[x].button_two.object->bottom_left.y =
      (button_group_y - 2.5) - button_height;
    button_groups[x].button_two.object->width = button_width - 10;
    button_groups[x].button_two.object->height = button_height - 10;
    center_x =
      button_groups[x].button_two.object->bottom_left.x + (button_width / 2);
    center_y =
      button_groups[x].button_two.object->bottom_left.y + (button_height / 2);

    button_groups[x].button_two.label->position = {
      (float) center_x - (button_groups[x].button_two.label->width / 2.0f) - 5,
      (float) center_y - (button_groups[x].button_two.label->height) + 5
    };
    button_group_x -= button_width;
...
void hmi_push_button_group(std::string b1, std::string b2)
{
  ...
  group.button_one.object = globals->renderer->PushPrimitive(
    new EasyPrimitive::Box({ -1000000, -1000000 }, 1, 1, 5));
  group.button_one.object->properties->mouse_callback = &hmi_mouse_callback;
  globals->renderer->SetColorByName(group.button_one.object->properties->color,
                                    "black");
  group.button_one.object->properties->zindex = 200;
  group.button_one.object->properties->id = b1;
  group.button_one.label = globals->renderer->PushPrimitive(
    new EasyPrimitive::Text({ -1000000, -1000000 }, group.button_one.name, 20));
  group.button_one.label->properties->zindex = 210;
  globals->renderer->SetColorByName(group.button_one.label->properties->color,
                                    "white");

  group.button_two.name = b2;
  group.button_two.object = globals->renderer->PushPrimitive(
    new EasyPrimitive::Box({ -1000000, -1000000 }, 1, 1, 5));
  group.button_two.object->properties->mouse_callback = &hmi_mouse_callback;
  globals->renderer->SetColorByName(group.button_two.object->properties->color,
                                    "black");
  group.button_two.object->properties->zindex = 200;
  group.button_two.object->properties->id = b2;
  group.button_two.label = globals->renderer->PushPrimitive(
    new EasyPrimitive::Text({ -1000000, -1000000 }, group.button_two.name, 20));
  group.button_two.label->properties->zindex = 210;
  globals->renderer->SetColorByName(group.button_two.label->properties->color,
                                    "white");
...
void hmi_up_key_callback(const nlohmann::json& e)
{
  try {
    nlohmann::json dro_data = motion_controller_get_dro();
...
void hmi_down_key_callback(const nlohmann::json& e)
{
  try {
    nlohmann::json dro_data = motion_controller_get_dro();
    if (dro_data["STATUS"] == "IDLE") {
...
void hmi_right_key_callback(const nlohmann::json& e)
{
  try {
    nlohmann::json dro_data = motion_controller_get_dro();
    if (dro_data["STATUS"] == "IDLE") {
...
void hmi_left_key_callback(const nlohmann::json& e)
{
  try {
    nlohmann::json dro_data = motion_controller_get_dro();
    if (dro_data["STATUS"] == "IDLE") {
      ...
      if (dro_data["STATUS"] == "IDLE")
      {
          if ((int)e["action"] == 1 || (int)e["action"] == 2)
          {
              ...
              else
              {
                  ...
                  motion_controller_push_stack("G53 G0 Z" +
  std::to_string(globals->nc_control_view->machine_parameters.machine_extents[2]));
                  motion_controller_push_stack("M30");
                  motion_controller_run_stack();
              }
          }
...
void hmi_init()
{
  ...
  globals->nc_control_view->machine_plane->properties->color[0] =
    globals->nc_control_view->preferences.machine_plane_color[0] * 255.0f;
  globals->nc_control_view->machine_plane->properties->color[1] =
    globals->nc_control_view->preferences.machine_plane_color[1] * 255.0f;
  globals->nc_control_view->machine_plane->properties->color[2] =
    globals->nc_control_view->preferences.machine_plane_color[2] * 255.0f;
  globals->nc_control_view->machine_plane->properties->matrix_callback =
    globals->nc_control_view->view_matrix;
  globals->nc_control_view->machine_plane->properties->mouse_callback =
    &hmi_mouse_callback;

  globals->nc_control_view->cuttable_plane =
    globals->renderer->PushPrimitive(new EasyPrimitive::Box(
      { globals->nc_control_view->machine_parameters.cutting_extents[0],
        globals->nc_control_view->machine_parameters.cutting_extents[1] },
      (globals->nc_control_view->machine_parameters.machine_extents[0] +
  ...
  globals->nc_control_view->cuttable_plane->properties->color[0] =
    globals->nc_control_view->preferences.cuttable_plane_color[0] * 255.0f;
  globals->nc_control_view->cuttable_plane->properties->color[1] =
    globals->nc_control_view->preferences.cuttable_plane_color[1] * 255.0f;
  globals->nc_control_view->cuttable_plane->properties->color[2] =
    globals->nc_control_view->preferences.cuttable_plane_color[2] * 255.0f;
  globals->nc_control_view->cuttable_plane->properties->matrix_callback =
    globals->nc_control_view->view_matrix;

  hmi_backpane = globals->renderer->PushPrimitive(
    new EasyPrimitive::Box({ -100000, -100000 }, 1, 1, 5));
  ...
  hmi_dro_backpane = globals->renderer->PushPrimitive(
    new EasyPrimitive::Box({ -100000, -100000 }, 1, 1, 5));
  hmi_dro_backpane->properties->color[0] = 29;
  hmi_dro_backpane->properties->color[1] = 32;
  hmi_dro_backpane->properties->color[2] = 48;
  ...
  hmi_button_backpane = globals->renderer->PushPrimitive(
    new EasyPrimitive::Box({ -100000, -100000 }, 1, 1, 5));
  ...
  dro.x.label = globals->renderer->PushPrimitive(
    new EasyPrimitive::Text({ -100000, -100000 }, "X", 50));
  dro.x.label->properties->zindex = 210;
  globals->renderer->SetColorByName(dro.x.label->properties->color, "white");
  dro.x.work_readout = globals->renderer->PushPrimitive(
    new EasyPrimitive::Text({ -100000, -100000 }, "0.0000", 40));
  ...
  dro.x.absolute_readout = globals->renderer->PushPrimitive(
    new EasyPrimitive::Text({ -100000, -100000 }, "0.0000", 15));
  ...
  dro.x.divider = globals->renderer->PushPrimitive(
    new EasyPrimitive::Box({ -100000, -10000 }, 1, 1, 3));
  dro.x.divider->properties->zindex = 150;
  globals->renderer->SetColorByName(dro.x.divider->properties->color, "black");

  dro.y.label = globals->renderer->PushPrimitive(
    new EasyPrimitive::Text({ -100000, -100000 }, "Y", 50));
  dro.y.label->properties->zindex = 210;
  globals->renderer->SetColorByName(dro.y.label->properties->color, "white");
  dro.y.work_readout = globals->renderer->PushPrimitive(
    new EasyPrimitive::Text({ -100000, -100000 }, "0.0000", 40));
  ...
  dro.y.absolute_readout = globals->renderer->PushPrimitive(
    new EasyPrimitive::Text({ -100000, -100000 }, "0.0000", 15));
  ...
  dro.z.absolute_readout = globals->renderer->PushPrimitive(
    new EasyPrimitive::Text({ -100000, -100000 }, "0.0000", 15));
  ...
  dro.z.divider = globals->renderer->PushPrimitive(
    new EasyPrimitive::Box({ -100000, -10000 }, 1, 1, 3));
  dro.z.divider->properties->zindex = 150;
  globals->renderer->SetColorByName(dro.z.divider->properties->color, "black");

  dro.feed = globals->renderer->PushPrimitive(
    new EasyPrimitive::Text({ -100000, -100000 }, "FEED: 0", 12));
  ...
  dro.arc_readout = globals->renderer->PushPrimitive(
    new EasyPrimitive::Text({ -100000, -100000 }, "ARC: 0.0V", 12));
  dro.arc_readout->properties->zindex = 210;
  dro.arc_readout->properties->color[0] = 247;
  dro.arc_readout->properties->color[1] = 104;
  dro.arc_readout->properties->color[2] = 15;

  dro.arc_set = globals->renderer->PushPrimitive(
    new EasyPrimitive::Text({ -100000, -100000 }, "SET: 0", 12));
  ...
  dro.run_time = globals->renderer->PushPrimitive(
    new EasyPrimitive::Text({ -100000, -100000 }, "RUN: 0:0:0", 12));
  ...
  globals->nc_control_view->torch_pointer = globals->renderer->PushPrimitive(
    new EasyPrimitive::Circle({ -100000, -100000 }, 5));
